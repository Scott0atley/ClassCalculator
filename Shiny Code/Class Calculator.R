                  ...
                  mainPanel(navbarPage('Class Calculator', id = "inTabset", 
                                       ...
                                       tabPanel(title="Class Schemas", value = "panel1",
                                                ...
                                                tags$h3("What do each of these schemas mean?"),
                                                  ...
                                                  p("The survey used a range of questions to measure three types of Capital: (1) economic capital (wealth and income, especially housing wealth), (2) cultural capital (of which there are two types: highbrow and emerging), (3) social capital (who people know, and the status of who they know). The survey drew heavily on the work of Bourdieu in its design. One of the key aims was to measure the three types of ‘capital’. There was a belief that social class in Britain today is a matter of advantages that people have accumulated over decades, and even generations as economic, cultural and social capital are passed down to children."),
                                                
                                                  tags$h4("What is the Wright Simplified Schema?"),
                                                  p("Erik Olin Wright’s model of social stratification can be described as a materialist and neo-Marxist conceptualisation of class structure with occasional Weberian leanings. Wright’s class schema is fundamentally based on the classification of economic agents."),
                                                
                                                  tags$h4("What is the NS-SEC Schema?"),
                                                  p("The National Statistics Socio-economic classification (NS-SEC) seeks to measure the employment relations and conditions of occupations. It is an adaptation of the Goldthorpe schema which was first known as the Nuffield Class Schema developed in the 1970s. It is is the official socio-economic classification in the United Kingdom."),
                                       ...
                                       tabPanel(title="GBCS", value = "panel2",
                                                conditionalPanel(
                                                  ...
                                                  tags$div(
                                                    style = "position: fixed; right: 0; top: 0; height: 100%; width: 600px; overflow-y: auto; padding-right: 25px;",
                                                    plotOutput("plot")
...
server <- function(input, output, session) {
  ...
  # This section creates the GBCS Classic BarPlot 
  
  # Generate circular bar plot based on user input
  output$plot <- renderPlot({
    # Calculate the adjusted values for homevalue and savings
    adjusted_homevalue <- as.numeric(input$homevalue) * 0.30
    adjusted_savings <- as.numeric(input$savings) * 0.70
    
    # Calculate the sum and mean values for social
    adjusted_sum <- length(input$social)
    adjusted_mean <- mean(as.numeric(input$social))
    
    # Normalize the values
    normalized_mean <- adjusted_mean / 85.30
    normalized_sum <- adjusted_sum / 18
    
    # Calculate the sum values for high
    adjusted_sumh <- length(input$high)
    
    # Normalize the values
    normalized_sumh <- adjusted_sumh / 8
    
    # Calculate the sum values for emerging
    adjusted_sume <- length(input$emerging)
    
    # Normalize the values
    normalized_sume <- adjusted_sume / 8
    
    # Create a data frame based on user inputs
    data <- data.frame(Category = c("Income", "Home Value", "SocialMean", "SocialSum", "HighSum", "EmergingSum"),
                       You = c("", "Economic", "Social", "", "Cultural", ""),
                       Value = c(as.numeric(input$income), adjusted_homevalue + adjusted_savings, normalized_mean, normalized_sum, normalized_sumh, normalized_sume))
    
    # Create the semi-circle donut chart
    df <- data %>%
      mutate(prop = Value / sum(Value),
             p_end = cumsum(prop),
             p_start = c(0, head(p_end, -1))) %>%
      mutate_at(c("p_start", "p_end"), rescale, to = pi * c(-0.5, 0.5), from = 0:1)
    
    # Plot the semi-circle donut chart
    ggplot(df, aes(x0 = 0, y0 = 0, r0 = 0.5, r = 1, fill = Category)) +
      geom_arc_bar(aes(start = p_start, end = p_end), color = "white", size = 0.5) +
      geom_text(aes(x = (p_start + p_end) / 2, y = 1.2, label = You), size = 4, color = "black", fontface = "bold") +
      coord_fixed() +
      theme_void() +
      scale_fill_manual(values = c("Income" = "purple4", "Home Value" = "purple4", "SocialMean" = "deeppink1", "SocialSum" = "deeppink1", "HighSum" = "turquoise2", "EmergingSum" = "turquoise2"), 
                        guide = "none") +
      labs(fill = "Category") +
